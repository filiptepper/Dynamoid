...
describe Dynamoid::AdapterPlugin::AwsSdkV2 do
  ...
  context 'with a preexisting table' do
    ...
    it 'performs scan on a table and returns items' do
      ...
      expect(Dynamoid.adapter.scan(test_table1, :name => { range_eq: 'Josh' }).to_a).to eq [{ :id=> '1', :name=>"Josh" }]
    end
    ...
    it 'performs scan on a table and returns multiple items if there are multiple matches' do
      ...
      expect(Dynamoid.adapter.scan(test_table1, :name => { range_eq: 'Josh' })).to include({:name=>"Josh", :id=>"2"}, {:name=>"Josh", :id=>"1"})
    end
